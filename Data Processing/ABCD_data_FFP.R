...
setwd("C:/Users/Admin/OneDrive - Maynooth University/Maynooth 2023/ABCD_Release_5_Core/Release 5.0")
...
ABCD_all_ID <- read.csv("C:/Users/cwyse/OneDrive - Maynooth University/maynooth 2023/ABCD_Release_5_Core/study documents/ABCD_all_ID.csv")
...
allfiles <- list.files("C:/Users/cwyse/OneDrive - Maynooth University/maynooth 2023/ABCD_Release_5_Core/Release 5.0", full.names = T, recursive = T, pattern = ".*.csv")
...
getvar <- function(table_to_find, var_to_find, mydata){
  ...
  myfile <- list.files("C:/Users/cwyse/OneDrive - Maynooth University/maynooth 2023/ABCD_Release_5_Core/Release 5.0", full.name=T, recursive = TRUE, pattern=table_to_find)
  myfile <- fread (myfile)
...
#----------------------- add variables for ABCD FFP Grant pilot data ---------------------

#abcd_yssbpm01              positive affect
#cbcl_scr_syn_anxdep_r      psychiatric diagnosis
table_to_find <- "mh_p_cbcl.csv"              # the table to find
var_to_find <- "cbcl_scr_syn_anxdep_r"        # the variable to find
mydata <- FFP  

#open mental health table
mh_p_cbcl <- read.csv("C:/Users/Admin/OneDrive - Maynooth University/Maynooth 2023/ABCD_Release_5_Core/Release 5.0/core/mental-health/mh_p_cbcl.csv")


table_m1 <- data.frame(src_subject_id = mh_p_cbcl$src_subject_id,
                       eventname = mh_p_cbcl$eventname,
                       cbcl_scr_syn_anxdep_r = mh_p_cbcl$cbcl_scr_syn_anxdep_r,
                       cbcl_scr_syn_anxdep_t = mh_p_cbcl$cbcl_scr_syn_anxdep_t
)

#import core variable data and make a new FFP dataset
FFP <- read.csv("C:/Users/Admin/OneDrive - Maynooth University/Maynooth 2023/ABCD_Release_5_Core/ABCD5_core_variables_060723.csv")

#merge the new variable into FPP data table
FFP <- merge(FFP, table_m1, by = c("src_subject_id", "eventname"))

# sleep duration variable
head(FFP$sleepdisturb1_p)
str(FFP$sleepdisturb1_p)
FFP$sleepdisturb1_p <- factor(FFP$sleepdisturb1_p)
sleep_hr <- FFP$sleepdisturb1_p
# 1 = 9-11 hours
# 2 = 8-9 hours
# 3 = 7-8 hours
# 4 = 5-7 hours 
...
head(FFP$sds_p_ss_total)
str(FFP$sds_p_ss_total)
sleep_dis <- FFP$sds_p_ss_total

#total Score (Sum of 6 Factors): sds_p_ss_dims + sds_p_ss_sbd + sds_p_ss_da + sds_p_ss_swtd + sds_p_ss_does + sds_p_ss_shy; 

#sex
head(FFP$sex)
FFP$sex <- factor(FFP$sex)  #1 = Male Masculino; 2 = Female Femenino
table(FFP$sex)
...
table(FFP$eventname)
str(FFP$eventname)
FFP$eventname <- factor(FFP$eventname)
event <- FFP$eventname

#mental health summary AnxDep CBCL Syndrome Scale (raw score) and t score
mental <- FFP$cbcl_scr_syn_anxdep_r
mental_t <- FFP$cbcl_scr_syn_anxdep_t

#mental health summary total CBCL Syndrome Scale (raw score) and t score
mental <- FFP$cbcl_scr_syn_totprob
mental_t <- FFP$cbcl_scr_syn_totprobt

mental_total <- mh_p_cbcl$cbcl_scr_syn_totprob_r
mental_total_t <- mh_p_cbcl$cbcl_scr_syn_totprob_t
...
plot(sleep_dis, mental)
plot(sleep_hr, mental)
plot(sleep_dis, mental_t)
plot(sleep_hr, mental_t)

data <- data.frame(event, sleep_hr, sleep_dis, mental, mental_t, mental_total, mental_total_t, sex)
basedata <- data[(data$event=="baseline_year_1_arm_1"),]
year2_data <- data[(data$event=="2_year_follow_up_y_arm_1"),]

basedata <- na.omit(basedata)
year2_data <- na.omit(year2_data)


# Function to calculate mean and confidence interval
mean_ci <- function(x) {
  mean <- mean(x, na.rm = TRUE)
  ci <- sd(x, na.rm = TRUE) / sqrt(length(x))
  return(data.frame(y = mean, ymin = mean - 1.96*ci, ymax = mean + 1.96*ci))
}
...
sleep_stats <- tapply(basedata$sleep_hr, mean_se)
mental_stats <- tapply(basedata$mental, mean_se)

# Create boxplots
boxplot( mental  ~ sleep_hr ~ , data = basedata, ylim = c(0, 10), outline = FALSE, main = "Sleep Grouped by Event")
points(sleep_stats, pch = 19, col = "red")
arrows(x0 = 1:2, y0 = sleep_stats[1, ] - sleep_stats[2, ], y1 = sleep_stats[1, ] + sleep_stats[2, ], 
       angle = 90, code = 3, length = 0.1, col = "red")

table(
  quantile(data$sleep_dis, probs = seq (0,1,0.2), na.rm=TRUE)
  ,data$mental_total)


# Calculate quintiles of 'sleep' variable
basedata$sleep_disQ5 <- factor(cut(basedata$sleep_dis, breaks = quantile(basedata$sleep_dis, probs = seq(0, 1, 0.2)), include.lowest = TRUE, labels = FALSE))
year2_data$sleep_disQ5 <- factor(cut(year2_data$sleep_dis, breaks = quantile(year2_data$sleep_dis, probs = seq(0, 1, 0.2)), include.lowest = TRUE, labels = FALSE))
# Creating a custom color palette using viridis
color_palette <- viridis(6)# Plotting
hist(basedata$sleep_dis)

# Plotting
p <- ggboxplot(basedata, x = "sleep_hr", y = "mental_total",
               fill = "sleep_hr",
               outlier.shape = NA,
               palette = viridis(5)) +
  coord_cartesian(ylim = c(0, 140)) + # Set y-axis limits +
  scale_y_continuous(breaks = seq(0, 140, by = 20))  +# Set y-axis intervals
  labs(x = "Sleep Duration (hr)", y = "CBCL Syndrome Scale (total)") +
  theme(legend.position = "none",
        axis.title = element_text(size = 12, face = "bold"),
        axis.text = element_text(size = 8))   # Increase font size of axis titles
# Remove legend

ggsave(file="sleep_dur.svg", device = svglite::svglite, plot=p, width=3.5, height=3)

q <- ggboxplot(basedata, x = "sleep_disQ5", y = "mental_total",
               fill = "sleep_disQ5",
               outlier.shape = NA,
               palette = viridis(5, direction = -1)) +
  labs(x = "Sleep Disruption Score (quintiles)", y = "CBCL Syndrome Scale (total)") +
  theme(legend.position = "none",  # Remove legend
        axis.title = element_text(size = 12, face = "bold"),
        axis.text = element_text(size = 8)) +  # Increase font size of axis titles
  coord_cartesian(ylim = c(0, 90)) +  # Set y-axis limits
  scale_y_continuous(breaks = seq(0, 90, by = 10))  # Set y-axis intervals

q
ggsave(file="sleep_dis.svg", device = svglite::svglite, plot=q, width=3.5, height=3)



p <- ggboxplot(year2_data, x = "sleep_hr", y = "mental_total",
               fill = "sleep_hr",
               outlier.shape = NA,
               palette = viridis(5)) +
  coord_cartesian(ylim = c(0, 140)) + # Set y-axis limits +
...
ggsave(file="sleep_dur.svg", device = svglite::svglite, plot=p, width=4, height=3)
p
q <- ggboxplot(year2_data, x = "sleep_disQ5", y = "mental_total",
               fill = "sleep_disQ5",
               outlier.shape = NA,
               palette = viridis(5)) +
  labs(x = "Sleep Disruption Score (quintiles)", y = "CBCL Syndrome Scale (total)") +
  theme(legend.position = "none",  # Remove legend
        ...
        axis.text = element_text(size = 10)) +  # Increase font size of axis titles
  coord_cartesian(ylim = c(0, 90)) +  # Set y-axis limits
  scale_y_continuous(breaks = seq(0, 90, by = 10))  # Set y-axis intervals

q
ggsave(file="sleep_dis.svg", device = svglite::svglite, plot=q, width=4, height=3)
...
#--------------    get the Fitbit wake times    ---------------------------------

library (lubridate)

fitbit_2yr <- nt_y_fitb_slp_d[(nt_y_fitb_slp_d$event == "2_year_follow_up_y_arm_1"),]

#fitbit 2yr follow up fitbit wake times
fitbit_2yr$waketime <- ymd_hms(fitbit_2yr$fit_ss_wakeup_minutes)
head(fitbit_2yr$fit_ss_wakeup_minutes)
str(fitbit_2yr$waketime)

# Subtract 12 hours
fitbit_2yr$waketime <-fitbit_2yr$waketime - hours(12)

# Extract hours and minutes as decimal values
fitbit_2yr$waketime_hr <- hour(fitbit_2yr$waketime) + minute(fitbit_2yr$waketime) / 60
...
# Group by ID and calculate mean and standard error of waketime
summary_stats <- fitbit_2yr %>%
  group_by(src_subject_id) %>%
  summarise(
    mean_waketime = mean(waketime_hr),
    std_error = sd(waketime_hr) / sqrt(n()),
    sd = sd(waketime_hr)
  )
head(summary_stats)
...
summary_stats <- summary_stats %>%
  mutate(
    lower_ci = mean_waketime - qt(0.975, n() - 1) * std_error,
    upper_ci = mean_waketime + qt(0.975, n() - 1) * std_error,
    CV = (sd/mean_waketime)
  )

# Tidy up the results using broom::tidy
tidy_summary <- summary_stats %>%
  select(src_subject_id, mean_waketime, lower_ci, upper_ci, CV) %>%
  rename(
    Estimate = mean_waketime,
    `2.5%` = lower_ci,
    `97.5%` = upper_ci
  )

#merge sex, age and mental
